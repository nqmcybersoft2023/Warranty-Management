...
document.getElementById("btnThemNV").onclick = function (event) {
  ...
  var congtacVien = getValueUser();

  console.log(congtacVien);
  if (validateSmartPhone(congtacVien)) {
  //   var requiredFields = ['tenKH', 'cccd', 'phone', 'email', 'thetinDung', 'nganHang'];
  
  arrtenCTV.push(congtacVien);
    document.getElementById("formQLNV").reset();
    luuDuLieuLocalStorage("arrtenKH", arrtenCTV);
    hienThiDuLieu();
  }
...
function hienThiDuLieu(arr) {
  ...
  var content = arr.map(function (congtacVien) {
    var newtenCTV = new SmartPhone();
    Object.assign(newtenCTV, congtacVien);

    return `
      <tr>
            <td>${newtenCTV.tenCTV}</td>
            <td>${newtenCTV.chietKhau}</td>
             
           <td>
          <button onclick="xoaDuLieuNhanVien('${newtenCTV.tenCTV}')" class="btn btn-danger">Xoá</button>
          <button data-target="#myModal" data-toggle="modal" onclick="getInfoUser('${newtenCTV.tenCTV}')"  class="btn btn-warning ml-3">Sửa</button>
        </td>
      </tr>
    `;
  }).join('');
...
//-------------XOÁ DỮ LIỆU---------------
function xoaDuLieuNhanVien(tenCTV) {
  var index = arrtenCTV.findIndex(item => item.tenCTV === tenCTV);

  if (index !== -1) {
    arrtenCTV.splice(index, 1);
    luuDuLieuLocalStorage("arrtenCTV", arrtenCTV);
    hienThiDuLieu();
  } else {
...
function getInfoUser(tenCTV) {
  var tenCTVIndex = arrModel.find(sanPham => sanPham.tenCTV === tenCTV);

  if (tenCTVIndex) {
    var arrInput = document.querySelectorAll("#myModal input, #myModal select");
    for (var htmlDom of arrInput) {
      var id = htmlDom.id;
      htmlDom.value = tenCTVIndex[id];
    }
  }
...
function updateValueUser() {
  var tenCTV = getValueUser();

  for (var i = 0; i < arrtenCTV.length; i++) {
    if (tenCTV.tenKH === arrtenCTV[i].tenCTV) {
      arrtenCTV[i] = tenCTV;
    }
  }
